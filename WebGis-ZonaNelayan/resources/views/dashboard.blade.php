            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 sm:mb-10 px-2">
                <div>
                    <h2 class="font-black text-2xl sm:text-3xl text-slate-800 tracking-tight flex items-center gap-2 sm:gap-3">
                        <span class="bg-blue-600 text-white p-2 sm:p-2.5 rounded-xl sm:rounded-2xl shadow-lg shadow-blue-100">📊</span>
                        {{ __('Log Laporan Zona') }}
                    </h2>
                                ...
                                <div class="flex-1 p-5 sm:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 md:gap-8">
                                    <div class="flex-1">
                                        <div class="flex flex-wrap items-center gap-2 sm:gap-4 mb-3 sm:mb-4">
                                            @if($lap->kategori_zona == 1)
                                                <span class="bg-emerald-50 text-emerald-700 text-[10px] sm:text-xs font-black px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl border border-emerald-100 uppercase tracking-tighter">🟢 Zona Aman</span>
                                            @elseif($lap->kategori_zona == 2)
                                                <span class="bg-amber-50 text-amber-700 text-[10px] sm:text-xs font-black px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl border border-amber-100 uppercase tracking-tighter">🟡 Zona Rawan</span>
                                            @else
                                                <span class="bg-rose-50 text-rose-700 text-[10px] sm:text-xs font-black px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl border border-rose-100 uppercase tracking-tighter">🔴 Zona Larangan</span>
                                            @endif
                                    ...
                                    <div class="flex items-center gap-3 w-full md:w-auto pt-5 md:pt-0 border-t md:border-t-0 border-slate-50">
                                        ...
                                        <form action="{{ route('lapor.destroy', $lap->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus laporan ini secara permanen?');" class="flex-1 md:flex-none">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="w-full bg-slate-50 hover:bg-rose-50 text-slate-600 hover:text-rose-600 font-black py-3 px-5 sm:px-6 rounded-2xl transition duration-200 text-sm flex items-center justify-center gap-2 border border-transparent hover:border-rose-100">
                                                <span>🗑️</span> Hapus
                                            </button>
                                        </form>
                                    </div>
                        ...
                        <div id="edit-modal-{{ $lap->id }}" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm overflow-y-auto h-full w-full hidden z-[100] flex items-center justify-center p-4">
                            <div class="relative p-6 sm:p-10 w-full max-w-lg shadow-[0_20px_50px_rgba(8,_112,_184,_0.7)] rounded-[2rem] sm:rounded-[2.5rem] bg-white border border-slate-200 animate-in fade-in zoom-in duration-300">
                                <div class="text-center">
                                    <div class="w-20 h-20 sm:w-24 sm:h-24 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-6 sm:mb-8 text-3xl sm:text-4xl shadow-inner border border-blue-100">
                                        📄
                                    </div>
